...
设有向图或无向图具有n个结点,则用结点表,边表表示该有向图或者无向图

结点表:用数组或者单链表的形式存放所有的结点,如结点数n已经知道,则采用数组的形式,否则采用单链表的形式

边表:(边结点表):每条边用一个结点进行表示.同一个结点的所有的边形成它的边结点单链表

优点:内存 = 结点数 + 边数

缺点:确定i->j是否有边,最坏需耗费O(n)时间.无向图同一条边表示两条边表空间浪费一倍.有向图中寻找进入某结点的边,非常困难。
```
```c++
#弧的结构
...
```
由于链接表中，有向链接表寻找进入结点的边非常困难，我们建议建立逆链接表或者十字链表，无向图的链接表中存储空间要多使用一倍，则我们建议改用邻接多重表

所以无向图更适配邻接多重表，有向图更加适配十字链表

这是因为

1.无向图 + 邻接多重表

无向图的边是“无方向”的,邻接多重表让一条边只对应1个边结点,同时关联两个顶点的邻接链表,既避免了传统邻接表“一条边存两次”的冗余,又能高效操作边(修改/删除只需处理一个结点),完美匹配无向图的特性.

2.有向图 + 十字链表

有向图的边是“有方向”的,需要同时管理出弧(从当前顶点出发的边)和入弧(指向当前顶点的边).十字链表的顶点结点同时包含firstout(出弧表头)和firstin(入弧表头),弧结点也同时记录了出弧和入弧的后继指针,能高效实现.“出弧/入弧的快速遍历”,是为有向图量身设计的存储结构.

逆链接表中每个顶点链接的是指向该顶点的弧(逆链接表的结构和链接表的结构相差无几,则我们不再多进行介绍)
...
scanf(&G.vexnum,&G.arcnum,&Inclnfo);
for(i=0;i<G.vexnum;++i){
...
*p = {i,j,G.xlist[j].firstin,G.xlist[i].firstout,NULL};#聚合初始化
...
1.同一条边只对应一个边结点，避免数据冗杂

2.建立邻接多重表的时间复杂度与建立邻接表的相同，各种基本操作的实现与邻接表相似
...
2.广度优先遍历
#### 深度优先遍历 DFS 适用的数据结构:栈
理论粗解
要判断V的邻接点是否被访问
可以为每个顶点设立一个“访问标志 visited[w]”

代码
```c++
#DFS实现的代码
##使用变量说明：Boolean visited[MAX];
...
```
#### 广度优先搜索遍历图 适用的数据结构 队列

结点的邻接结点的次序是任意的,因此广度优先搜索的序列可能有多种

广度优先搜索类似于树的从根部出发的按照层次的遍历

代码

```c++
#使用的变量说明 Boolean visited[MAX];
...
for(v = 0;v<G.vexnum;++v)
if(! Visited[v])
{Visited[v] = TURE;Visit(v);EnQueue(Q,v);
while(!EmptyQueue(Q)}
{DeQueue(Q,u);
...
##### 生成树
极小连通子图.包含图的所有n个结点,但只含图的n-1条边任意两点之间,本来就是连通的,添加一条边之后,形成了这两个顶点之间的第二条通路

生成方法:进行深度为主的遍历或广度为主的遍历,得到深度优先生成树和广度优先生成树
##### 生成森林
在进行深度为主的遍历或者广度为主的遍历时,对于非连通图,将得到多棵深度优先生成树或者广度优先生成树,称之为生成森林

以孩子兄弟链表生成森林的存储结构,则算法7.7生成非连通图的深度优先生成森林.算法的复杂度和遍历相同.

##### 生成连通图的生成树
[生成方法]对图进行深度优先遍历或广度优先遍历,遍历的过程中经历的边的集合与图中的所有顶点一起构成图的一颗深度优先生成树或广度优先生成树
##### 非连通图的生成森林
[生成方法]对非连通图进行深度优先遍历或广度优先遍历时,每个连通分量广度顶点集和遍历时走过的边一起构成若干棵生成树,即生成森林
### 有向无环图及其运用
### 最短路径
# C++ 数据结构基础操作速查表
